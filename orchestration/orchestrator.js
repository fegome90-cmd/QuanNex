...
class WorkflowOrchestrator {
  ...
  validateWorkflowConfig(cfg) {
    ...
    for (const s of cfg.steps) {
      if (!s.step_id || ids.has(s.step_id)) throw new Error(`Duplicate or missing step_id: ${s.step_id}`);
      ids.add(s.step_id);
      if (!s.agent) throw new Error(`Step ${s.step_id} missing agent`);
      if (s.depends_on && !Array.isArray(s.depends_on)) throw new Error(`Step ${s.step_id} depends_on must be array`);
  ...
  async createWorkflow(workflowConfig) {
    ...
    const workflow = {
      ...
      steps: workflowConfig.steps.map(s => ({ ...s, status: 'pending', retry_count: 0 })),
      status: 'pending',
      created_at: nowIso(),
    ...
    writeFileSync(join(workflow.artifacts_dir, 'plan.json'), JSON.stringify(workflowConfig, null, 2));
    ...
    try {
      ...
      while (pending.size > 0) {
        const ready = workflow.steps.filter(s =>
          pending.has(s.step_id) && (!s.depends_on || s.depends_on.every(d => executedSteps.has(d)))
        );
        if (ready.length === 0) {
  ...
  async executeStep(workflow, step) {
    ...
    try {
      ...
      const result = await this.callAgent(step.agent, step.action || '', input, { timeoutMs: step.timeout_ms || this.timeoutMs });
      // NEW: gates
  ...
  async callAgent(agentName, action, input, { timeoutMs = DEFAULT_TIMEOUT_MS } = {}) {
    const runCleanPath = join(PROJECT_ROOT, 'scripts', 'run-clean.sh');
    if (!existsSync(runCleanPath)) {
      throw new Error(`run-clean.sh not found at ${runCleanPath}`);
    }

    return new Promise((resolve, reject) => {
      const child = spawn('bash', [runCleanPath, agentName, '--stdin-json', '--out', `out/${agentName}.json`], {
      ...
      child.on('close', (code) => {
        ...
        if (code === 0) {
          try {
            ...
            reject(new Error(`Parse error from ${agentName}: ${e.message}; stderr: ${stderr.slice(0, 500)}`));
          }
        } else {
  ...
  async healthCheck() {
    const health = {};
    for (const [agentName] of this.agentStatus) {
      try {
        // Send valid test input for each agent
        let testInput = {};
        switch (agentName) {
          case 'rules':
            testInput = {
        ...
        await this.callAgent(agentName, 'ping', testInput, { timeoutMs: 5_000 });
        health[agentName] = { status: 'healthy', lastCheck: nowIso() };
      } catch (e) {
        health[agentName] = { status: 'unhealthy', error: e.message, lastCheck: nowIso() };
      }
    }
    return health;
  }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  // Usar top-level await legal en ESM
  try {
    switch (command) {
      case 'create': {
        let configArg = process.argv[4];
        if (!configArg) {
          // Try to read from stdin
          const { readFileSync } = await import('node:fs');
          try {
            configArg = readFileSync(0, 'utf8');
          } catch {
            // eslint-disable-next-line no-console
            console.error('Configuration JSON required');
            process.exit(1);
          }
        }
      ...
      case 'execute': {
        ...
        const result = await orchestrator.executeWorkflow(arg3);
        // eslint-disable-next-line no-console
      ...
      case 'status': {
        if (arg3) {
          // eslint-disable-next-line no-console
          console.log(JSON.stringify(orchestrator.getWorkflow(arg3), null, 2));
        } else {
          // eslint-disable-next-line no-console
          console.log(JSON.stringify(orchestrator.listWorkflows(), null, 2));
        }
        break;
      }
      case 'health': {
        // eslint-disable-next-line no-console
        console.log(JSON.stringify(await orchestrator.healthCheck(), null, 2));
        break;
      }
      case 'cleanup': {
        if (!arg3) {
        ...
        const ok = orchestrator.cleanup(arg3);
        // eslint-disable-next-line no-console
        console.log(JSON.stringify({ workflow_id: arg3, cleaned: ok }, null, 2));
        break;
      }
      default:
        // eslint-disable-next-line no-console
        console.log('Usage: orchestrator.js <create|execute|status|health|cleanup> [workflowId] [jsonConfig]');
        process.exit(1);
    }
  } catch (e) {
    // eslint-disable-next-line no-console
