...
function testAgentSanitization(agentPath) {
  ...
  if (agentPath.includes('prompting')) {
    testCases = TEST_CASES.filter(
      tc =>
        tc.name.includes('prompting') ||
        tc.name.includes('path traversal') ||
        tc.name.includes('caracteres peligrosos') ||
        tc.name.includes('JSON malformado') ||
        tc.name.includes('max_tokens negativo')
    );
  } else if (agentPath.includes('rules')) {
    testCases = TEST_CASES.filter(
      tc =>
        tc.name.includes('rules') ||
        tc.name.includes('path traversal') ||
        tc.name.includes('caracteres peligrosos') ||
        tc.name.includes('JSON malformado') ||
        tc.name.includes('max_tokens negativo')
  ...
  for (const testCase of testCases) {
    ...
    try {
      ...
      if (testResult.actual === testCase.expected) {
        ...
        console.log(
          `    ❌ FAIL: ${testCase.name} - Expected ${testCase.expected}, got ${testResult.actual}`
        );
      }
