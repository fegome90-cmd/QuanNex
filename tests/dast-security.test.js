...
describe('DAST Security Testing System', () => {
  describe('DAST Scanner', () => {
    test('should show help when no arguments provided', async () => {
      const result = await runScript('./core/scripts/dast-scan.sh');
    ...
    test('should show help with --help flag', async () => {
      const result = await runScript('./core/scripts/dast-scan.sh', ['--help']);
    ...
    test('should perform basic scan on httpbin.org', async () => {
      ...
      const reportFiles = require('node:fs')
        .readdirSync(DAST_DIR)
        .filter(
          file => file.startsWith('dast-report-') && file.endsWith('.json')
        );
  ...
  describe('Security Dependencies Scanner', () => {
    test('should show help when no arguments provided', async () => {
      const result = await runScript('./core/scripts/security-deps-scan.sh', [
        '--help'
      ]);
    ...
    test('should perform security dependencies scan', async () => {
      ...
      const reportFiles = require('node:fs')
        ...
        .filter(
          file =>
            file.startsWith('security-deps-report-') && file.endsWith('.json')
        );
  ...
  describe('Security Report Aggregator', () => {
    ...
    test('should aggregate security reports', async () => {
      ...
      await runScript('./core/scripts/dast-scan.sh', [
        '-t',
        'basic',
        'https://httpbin.org'
      ]);

      await runScript('./core/scripts/security-deps-scan.sh', [
        '-l',
        'moderate'
      ]);
      ...
      const reportFiles = require('node:fs')
        ...
        .filter(
          file =>
            file.startsWith('consolidated-security-report-') &&
            file.endsWith('.json')
        );
  ...
  describe('NPM Scripts Integration', () => {
    test('should run dast:basic script', async () => {
      const result = await runScript('npm', [
        'run',
        'dast:basic',
        'https://httpbin.org'
      ]);
  ...
  describe('Report Structure Validation', () => {
    test('should generate valid JSON reports', async () => {
      ...
      const dastReports = require('node:fs')
        .readdirSync(DAST_DIR)
        .filter(
          file => file.startsWith('dast-report-') && file.endsWith('.json')
        );

      const securityReports = require('node:fs')
        .readdirSync(SECURITY_DIR)
        .filter(
          file =>
            file.startsWith('security-deps-report-') && file.endsWith('.json')
