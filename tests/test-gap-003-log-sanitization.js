  ...
  try {
    ...
    if (testResult.passed) {
      ...
      for (const finding of findings) {
        console.log(
          `      - ${finding.severity.toUpperCase()}: ${finding.type} - ${finding.match}`
        );
      }
    }
...
const report = {
  ...
  summary: {
    ...
    sensitive_findings: allResults.reduce(
      (sum, r) => sum + r.tests.reduce((s, t) => s + (t.findings ? t.findings.length : 0), 0),
      0
    ),
  },
};
