...
const report = {
  ...
  summary: {
    ...
    total_communication_patterns: allResults.reduce(
      (sum, r) => sum + r.communication_patterns.length,
      0
    ),
    total_auth_patterns: allResults.reduce((sum, r) => sum + r.auth_patterns.length, 0),
    unauthenticated_communications: allResults.reduce(
      (sum, r) => sum + r.communication_patterns.filter(p => !p.authenticated).length,
      0
    ),
  },
};
