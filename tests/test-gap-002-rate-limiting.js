  ...
  for (const testCase of testCases) {
    try {
      ...
      for (const concurrency of concurrencyLevels) {
        ...
        console.log(
          `    ${analysis.rate_limiting_detected ? '✅' : '❌'} ${concurrency} concurrent: ${analysis.success_rate}% success, ${analysis.avg_duration_ms}ms avg`
        );
  ...
  for (const result of results) {
    ...
    for (const test of result.tests) {
      ...
      if (test.analysis) {
        ...
        console.log(
          `    Rate Limiting: ${test.analysis.rate_limiting_detected ? 'DETECTED' : 'NOT DETECTED'}`
        );
