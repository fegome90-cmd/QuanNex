...
import { sanitizeObject } from '../../shared/utils/security.js';
...
export default class TaskRunner {
  ...
  enforceRateLimit(task) {
    const { windowMs, maxRequests } = this.rateLimitConfig;
    if (!maxRequests || maxRequests <= 0) {
      return;
    }
    const key = task?.metadata?.tenant || task?.metadata?.token || task?.thread_state_id || task?.intent || 'default';
    const now = Date.now();
    const bucket = this.rateLimiter.get(key) || { count: 0, windowStart: now };
