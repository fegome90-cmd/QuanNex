...
const SECRET_PATTERNS = [
  // Claves hardcodeadas
  { pattern: /secret\s*[:=]\s*["']?[^"'\s]+["']?/gi, type: 'hardcoded_secret', severity: 'critical' },
  { pattern: /key\s*[:=]\s*["']?[^"'\s]+["']?/gi, type: 'hardcoded_key', severity: 'critical' },
  { pattern: /password\s*[:=]\s*["']?[^"'\s]+["']?/gi, type: 'hardcoded_password', severity: 'critical' },
  { pattern: /token\s*[:=]\s*["']?[^"'\s]+["']?/gi, type: 'hardcoded_token', severity: 'critical' },
  { pattern: /api[_-]?key\s*[:=]\s*["']?[^"'\s]+["']?/gi, type: 'hardcoded_api_key', severity: 'critical' },
  
  // Claves de cifrado
  { pattern: /encryption[_-]?key\s*[:=]\s*["']?[^"'\s]+["']?/gi, type: 'encryption_key', severity: 'critical' },
  { pattern: /private[_-]?key\s*[:=]\s*["']?[^"'\s]+["']?/gi, type: 'private_key', severity: 'critical' },
  { pattern: /public[_-]?key\s*[:=]\s*["']?[^"'\s]+["']?/gi, type: 'public_key', severity: 'high' },
  
  // Credenciales de base de datos
  { pattern: /db[_-]?password\s*[:=]\s*["']?[^"'\s]+["']?/gi, type: 'db_password', severity: 'critical' },
  { pattern: /database[_-]?password\s*[:=]\s*["']?[^"'\s]+["']?/gi, type: 'database_password', severity: 'critical' },
  
  // URLs con credenciales
  { pattern: /https?:\/\/[^:]+:[^@]+@/gi, type: 'url_with_credentials', severity: 'critical' },
  
  // Variables de entorno mal usadas
  { pattern: /process\.env\.[A-Z_]+.*=.*["'][^"']+["']/gi, type: 'env_hardcoded', severity: 'high' }
];
  ...
  for (const filePath of FILES_TO_ANALYZE) {
    ...
    if (findings.secure_patterns.length > 0) {
      ...
      for (const pattern of findings.secure_patterns) {
        console.log(`    - ${pattern.security_level.toUpperCase()}: ${pattern.type} (line ${pattern.line})`);
      }
    }
...
function generateReport(results) {
  ...
  const security_ratio = results.total_secure_patterns / Math.max(results.total_hardcoded_secrets, 1);
