...
const validatePolicyRefs = (policyRefs, errors) => {
  ...
  if (!policyRefs.every((item) => typeof item === 'string' && item.trim() !== '')) {
...
const validateInput = (data) => {
  ...
  validateEnumField(data.compliance_level, 'compliance_level', ALLOWED_COMPLIANCE, errors);
...
const validateOutput = (data) => {
  ...
  if (!Array.isArray(data.rules_compiled) || !data.rules_compiled.every((item) => typeof item === 'string')) {
    errors.push('rules_compiled must be an array of strings');
  }
  if (data.violations !== undefined && (!Array.isArray(data.violations) || !data.violations.every((item) => typeof item === 'string'))) {
    errors.push('violations must be an array of strings when present');
  }
  if (data.advice !== undefined && (!Array.isArray(data.advice) || !data.advice.every((item) => typeof item === 'string'))) {
    errors.push('advice must be an array of strings when present');
  }
  if (data.trace !== undefined && (!Array.isArray(data.trace) || !data.trace.every((item) => typeof item === 'string'))) {
