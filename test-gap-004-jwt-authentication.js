...
function testAgentJWTImplementation(agentPath) {
  ...
  try {
    ...
    console.log(`  ✅ Token verified: ${payload.sub} with permissions: ${payload.permissions.join(', ')}`);
  ...
  results.overall = (passedTests === totalTests && jwtTestsPassed === totalJwtTests) ? 'pass' : 'fail';
...
const report = {
  ...
  summary: {
    ...
    passed_jwt_tests: allResults.reduce((sum, r) => sum + r.jwt_tests.filter(t => t.passed).length, 0)
  }
};
