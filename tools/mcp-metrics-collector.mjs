...
class MCPMetricsCollector {
  ...
  async analyzeComplete(targetPath) {
    ...
    if (optResult.success) {
      this.metrics.optimization.push({
        target: targetPath,
        total:
          optResult.result.optimization_report?.summary?.total_optimizations ||
          0,
        duration_ms: optResult.duration_ms
      });
      console.log(
        `✅ Optimizaciones: ${optResult.result.optimization_report?.summary?.total_optimizations || 0}`
      );
    }
    ...
    if (secResult.success) {
      this.metrics.security.push({
        target: targetPath,
        vulnerabilities:
          secResult.result.security_report?.summary?.vulnerabilities_found || 0,
        compliance:
          secResult.result.security_report?.summary?.compliance_score || 0,
        duration_ms: secResult.duration_ms
      });
      console.log(
        `✅ Vulnerabilidades: ${secResult.result.security_report?.summary?.vulnerabilities_found || 0}`
      );
      console.log(
        `✅ Compliance: ${secResult.result.security_report?.summary?.compliance_score || 0}/100`
      );
    }
    ...
    if (qualityResult.success) {
      this.metrics.quality.push({
        target: targetPath,
        quality_score:
          qualityResult.result.metrics_report?.quality?.quality_score || 0,
        coverage:
          qualityResult.result.metrics_report?.summary?.coverage_percentage ||
          0,
        duration_ms: qualityResult.duration_ms
      });
      console.log(
        `✅ Quality: ${qualityResult.result.metrics_report?.quality?.quality_score || 0}/100`
      );
    }
  ...
  generateReport() {
    return {
      ...
      summary: {
        total_optimizations: this.metrics.optimization.reduce(
          (sum, m) => sum + m.total,
          0
        ),
        total_vulnerabilities: this.metrics.security.reduce(
          (sum, m) => sum + m.vulnerabilities,
          0
        ),
        avg_compliance:
          this.metrics.security.reduce((sum, m) => sum + m.compliance, 0) /
          (this.metrics.security.length || 1),
        avg_quality:
          this.metrics.quality.reduce((sum, m) => sum + m.quality_score, 0) /
          (this.metrics.quality.length || 1)
      }
    };
  }
