...
const SLAS = {
  ...
  rules: { p95_max: 0.2 * 1.2, success_min: 1.0 }
};
...
for (const agent of ['context', 'prompting', 'rules']) {
  ...
  if (a.p95_s > SLAS[agent].p95_max)
    fail(`${agent}: p95 ${a.p95_s}s > ${SLAS[agent].p95_max}s`);
  if (a.success_rate < SLAS[agent].success_min)
    fail(`${agent}: success ${a.success_rate} < ${SLAS[agent].success_min}`);
  ok(`${agent}: p95=${a.p95_s}s, success=${a.success_rate}`);
}

if (latest.isolation.leaks !== ISOLATION.leaks)
  fail(
    `isolation.leaks=${latest.isolation.leaks} (esperado ${ISOLATION.leaks})`
  );
if (latest.isolation.temp_after_run !== ISOLATION.temp_after_run)
  fail(
    `isolation.temp_after_run=${latest.isolation.temp_after_run} (esperado ${ISOLATION.temp_after_run})`
  );
ok('Aislamiento OK (0 leaks, 0 residuos)');

if (latest.tests.passed < TESTS.min_pass)
  fail(`tests.passed=${latest.tests.passed} < ${TESTS.min_pass}`);
if (latest.tests.failed > TESTS.max_fail)
  fail(`tests.failed=${latest.tests.failed} > ${TESTS.max_fail}`);
