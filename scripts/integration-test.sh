...
prepare_project_dir() {
  ...
  if [[ ! -d "$PROJECT_DIR/.claude/commands" ]] || [[ ! -f "$PROJECT_DIR/CLAUDE.md" ]]; then
    ...
    CLAUDE_INIT_SKIP_DEPS=1 CLAUDE_INIT_INCLUDE_DEPLOY=1 \
      bash "$ROOT/claude-project-init.sh" --name demo --type fullstack --use-templates on --templates-path "$ROOT/templates" --path "$TMP" --yes >/dev/null 2>&1 || {
...
test_agent() {
  ...
  case "$agent" in
    "code-reviewer")
      if run_step "agent-${agent}" ./scripts/eslint-check.sh "$project_dir"; then
      ...
      ;;
    "medical-reviewer")
      if run_step "agent-${agent}" ./scripts/check-phi.sh "$project_dir"; then
      ...
      ;;
    "security-guardian")
      if run_step "agent-${agent}" ./scripts/security-scan.sh "$project_dir"; then
      ...
      ;;
    "deployment-manager")
      if run_step "agent-${agent}" ./scripts/deployment-check.sh "$project_dir"; then
      ...
      ;;
    "test-generator")
      if run_step "agent-${agent}" ./scripts/test-generator.sh "$project_dir"; then
      ...
      ;;
    "review-coordinator")
      if ./scripts/review-coordinator.sh "$project_dir" >/dev/null 2>&1; then
...
test_workflow() {
  ...
  if ! run_step "workflow-eslint" ./scripts/eslint-check.sh "$PROJECT_DIR"; then
  ...
  if ! run_step "workflow-security" ./scripts/security-scan.sh "$PROJECT_DIR"; then
  ...
  if ! run_step "workflow-deploy" ./scripts/deployment-check.sh "$PROJECT_DIR"; then
  ...
  if ! run_step "workflow-testgen" ./scripts/test-generator.sh "$PROJECT_DIR"; then
  ...
  if ! PROJECT_DIR="$PROJECT_DIR" REVIEW_COORDINATOR_EXECUTE=0 NO_COLOR=1 ./scripts/review-coordinator.sh "$PROJECT_DIR" >"$LOG_DIR/workflow-review-coordinator.log" 2>&1; then
