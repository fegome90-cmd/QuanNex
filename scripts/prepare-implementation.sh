...
- repo: local
  ...
  - id: quality-check
    name: Quality Check
    entry: ./scripts/quality/quality-check.sh
    language: system
    pass_filenames: false
    always_run: true
EOF
...
create_quality_scripts() {
  ...
  cat >"$PROJECT_ROOT/scripts/quality/quality-check.sh" <<'EOF'
#!/bin/bash
# Script de verificación de calidad
# Archivo generado automáticamente por prepare-implementation.sh
...
EOF
  ...
  # Script de métricas
  cat >"$PROJECT_ROOT/scripts/quality/metrics.sh" <<'EOF'
#!/bin/bash
...
EOF
  ...
  chmod +x "$PROJECT_ROOT/scripts/quality/quality-check.sh"
  chmod +x "$PROJECT_ROOT/scripts/quality/metrics.sh"
...
jobs:
  prepare:
    ...
    - name: Check implementation preparation
      run: |
        chmod +x scripts/quality/quality-check.sh
...
prepare: ## Preparar entorno para implementación
	...
	@chmod +x scripts/prepare-implementation.sh
	@./scripts/prepare-implementation.sh
...
backup: ## Crear backup del proyecto
	...
	@cp -r claude-project-init.sh .claude scripts docs Makefile README.md backup-$(shell date +%Y%m%d-%H%M%S)/

# Comandos de Archon (mantenidos del Makefile original)
archon-bootstrap:
	@bash scripts/archon-bootstrap.sh

archon-check:
	@bash scripts/archon-setup-check.sh $(ARCHON_DIR)

archon-smoke:
	@bash scripts/archon-smoke.sh $(ARCHON_DIR)
...
implementation-status: ## Mostrar estado de la implementación
	...
	@echo "  🔧 Scripts de calidad: $(shell [ -f scripts/quality/quality-check.sh ] && echo "✅ Creados" || echo "❌ Pendientes")"
EOF
...
- `src/templates/` - Templates de proyecto
...
main() {
  ...
  chmod +x "$PROJECT_ROOT/scripts/quality/metrics.sh"
  "$PROJECT_ROOT/scripts/quality/metrics.sh"
