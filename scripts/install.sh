...
rsync -av --exclude='scripts/install.sh' "$KIT_DIR/" "$INSTALL_DIR/"

# Set permissions
echo "  🔧 Configurando permisos..."
chmod +x "$INSTALL_DIR/claude-project-init.sh"
...
if [[ $create_symlink =~ ^[Yy]$ ]]; then
  ...
  if [ -w "$SYMLINK_DIR" ]; then
    ln -sf "$INSTALL_DIR/claude-project-init.sh" "$SYMLINK_DIR/claude-project-init"
    ...
    echo "Ejecuta manualmente: sudo ln -sf '$INSTALL_DIR/claude-project-init.sh' '/usr/local/bin/claude-project-init'"
  fi
fi
...
echo -e "  ${YELLOW}$INSTALL_DIR/claude-project-init.sh${NC}     # Crear proyecto"
echo -e "  ${YELLOW}$INSTALL_DIR/scripts/verify-dependencies.sh${NC}  # Verificar deps"
echo -e "  ${YELLOW}$INSTALL_DIR/scripts/test-claude-init.sh${NC}      # Testing completo"
...
echo -e "\n💡 Siguiente paso:"
echo -e "${GREEN}cd $INSTALL_DIR && ./claude-project-init.sh${NC}"
