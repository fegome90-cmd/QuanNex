...
# Test script for claude-project-init.sh
...
SCRIPT_PATH="$REPO_ROOT/claude-project-init.sh"
...
test_project_creation() {
  ...
  echo -e "$project_name\n$project_type" | CLAUDE_INIT_SKIP_DEPS=1 bash "$SCRIPT_PATH" --use-templates on --templates-path "$REPO_ROOT/templates" >/dev/null 2>&1
...
# Main test execution
main() {
  log "Starting comprehensive test of claude-project-init.sh"
  ...
  if [ "$TESTS_FAILED" -eq 0 ]; then
    ...
    echo -e "${GREEN}✅ The claude-project-init.sh script works perfectly with all 6 project types.${NC}"
