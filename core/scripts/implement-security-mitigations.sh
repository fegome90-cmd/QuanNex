...
EOF
  ...
  if grep -q "docs/reference" core/scripts/security-scan.sh; then
  ...
  if command -v ./core/scripts/security-scan.sh &>/dev/null; then
    if SECURITY_STRICT=0 ./core/scripts/security-scan.sh . 2>/dev/null; then
