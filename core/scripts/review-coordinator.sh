...
execute_agent() {
  ...
  case "$agent" in
    "code-reviewer")
      if [[ -f "core/scripts/eslint-check.sh" ]]; then
        ./core/scripts/eslint-check.sh "$root"
      else
        warning "Script de code-reviewer no encontrado"
      fi
      ;;
    "medical-reviewer")
      if [[ -f "core/scripts/check-phi.sh" ]]; then
        ./core/scripts/check-phi.sh "$root"
      else
        warning "Script de medical-reviewer no encontrado"
      fi
      ;;
    "security-guardian")
      if [[ -f "core/scripts/security-scan.sh" ]]; then
        ./core/scripts/security-scan.sh "$root"
      else
        warning "Script de security-guardian no encontrado"
      fi
      ;;
    "deployment-manager")
      if [[ -f "core/scripts/deployment-check.sh" ]]; then
        ./core/scripts/deployment-check.sh "$root"
      else
        warning "Script de deployment-manager no encontrado"
      fi
      ;;
    "test-generator")
      if [[ -f "core/scripts/test-generator.sh" ]]; then
        ./core/scripts/test-generator.sh "$root"
      else
